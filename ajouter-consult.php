                         ...
                         <div class="content">
                             <form  method="post">
                                   ...
                                   <div class="input-box">
                                        ...
                                        <input type="text" placeholder="Entrer le motif de la consultation" name="motif" required>
                                    </div>

                                    <div class="input-box">
                                        <span class="details" > conclusion </span>
...
$database =new PDO("mysql:host=localhost; dbname=dbcabinet",$username,$password);
 ...
 $m= htmlspecialchars($_POST["motif"]);

 $c= htmlspecialchars($_POST["conclusion"]);
 $a = $database->prepare("SELECT * from patient,rdv WHERE patient.idpatient=rdv.idpatient AND nom=:nom AND prenom=:prenom");
 $a->bindParam(":nom",$_POST["nom"]);
 $a->bindParam(":prenom",$_POST["prenom"]);
 ...
 $ad = $database->prepare("SELECT * from patient,ordonance WHERE patient.idpatient=ordonance.idpatient AND nom=:nom AND prenom=:prenom");
 ...
 $ad->execute();
  ...
  $gtline = $database->prepare("INSERT INTO consultation (motif,conclusion,idpatient,idrdv,idord) VALUES(:motif,:conclusion,:idpatient,:idrdv,:idord)");
    
       $gtline->bindParam(":motif",$m);
       $gtline->bindParam(":conclusion",$c);
       $gtline->bindParam(":idpatient",$_SESSION["id"]->idpatient);
       $gtline->bindParam(":idrdv",$_SESSION["qq"]->idrdv);
       $gtline->bindParam(":idord",$_SESSION["id"]->idord);
       $gtline->execute();
       
 echo"<script> alert(' consultation ajoutée ') </script>";
