...
$ge = $database->prepare("SELECT * FROM patient,consultation,ordonance where patient.idpatient=consultation.idpatient and ordonance.idord=consultation.idord ");
...
<div class="sidebar">
    ...
    <ul>
          ...
          <ul class="submenu"><li><a href="http://localhost/atelier/ajouter-rdv.php">ajouter un rendez-vous </a></li> </li><li><a href="localhost/atelier/modifier-rdv.php">modifier un rendez-vous</a></li> </ul>
          </li>
...
<div class="main-content">
     ...
     <tr>
       ...
       <th width="260px">Prenom</th>
       <th width="260px">Date</th>
      <th width="260px"> conclusion </th>
      <th width="260px"></th>
     </tr>
      </table>';
      ...
      foreach($ge AS $data){
       ...
       <tr>
          ...
          <td width='260px'>".$data["prenom"]."</td>
          <td width='260px'>".$data["dateord"]."</td>
          <td width='260px'>".$data["conclusion"]."</td>
      
          <td width='260px'> <button  name='remove'value='".$data["idcons"]."'>supp</button></td>
        </tr>
        </table></form>";};
        echo'
...
$database =new PDO("mysql:host=localhost; dbname=dbcabinet",$username,$password);
 ...
 $ad = $database->prepare("DELETE FROM consultation WHERE idcons=:id");
 $ad->bindParam(":id",$data["idcons"]); 
 ...
 $ad->execute();
 echo"<script> alert(' Patient supprimé avec succé ') </script>";
